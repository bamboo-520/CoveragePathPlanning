...
void Dijkstra::init(ros::NodeHandle& nh)
{
  // 2d参数
  nh.param("global_planner/is_2D", is_2D, 0);  // 1代表2D平面规划及搜索,0代表3D
  nh.param("global_planner/2D_fly_height", fly_height, 1.5);  // 2D规划时,定高高度
  // 规划搜索相关参数
  lambda_heu_ = 0.0;  // Dijkstra 不使用启发式项
  nh.param("dijkstra/allocate_num", max_search_num, 100000); // 最大搜索节点数
  // 地图参数
...
// 搜索函数，输入为：起始点及终点
// 将传输的数组通通变为指针！！！！ 以后改
int Dijkstra::search(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
  // 首先检查目标点是否可到达
  if(Occupy_map_ptr->getOccupancy(end_pt))
  {
    pub_message(message_pub, prometheus_msgs::Message::WARN, NODE_NAME, "Dijkstra can't find path: goal point is occupied.");
    return NO_PATH;
  }
      ...
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
      {
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_)
        {
          ...
          if (expand_node == NULL)
          {
            ...
            {
                pub_message(message_pub, prometheus_msgs::Message::WARN, NODE_NAME, "Dijkstra can't find path: reach the max_search_num.\n");
                return NO_PATH;
            }
          }
          // 如果当前节点已被扩展过，则更新其状态
...
// 由最终点往回生成路径
void Dijkstra::retrievePath(NodePtr end_node)
{
...
std::vector<Eigen::Vector3d> Dijkstra::getPath()
{
...
nav_msgs::Path Dijkstra::get_ros_path()
{
  nav_msgs::Path path;
...
double Dijkstra::getDiagHeu(Eigen::Vector3d x1, Eigen::Vector3d x2)
{
...
double Dijkstra::getManhHeu(Eigen::Vector3d x1, Eigen::Vector3d x2)
{
  double dx = fabs(x1(0) - x2(0));
  double dy = fabs(x1(1) - x2(1));
  double dz = fabs(x1(2) - x2(2));
...
double Dijkstra::getEuclHeu(Eigen::Vector3d x1, Eigen::Vector3d x2)
{
  return tie_breaker_ * (x2 - x1).norm();
}

std::vector<NodePtr> Dijkstra::getVisitedNodes()
{
...
Eigen::Vector3i Dijkstra::posToIndex(Eigen::Vector3d pt)
{
  Vector3i idx ;
...
void Dijkstra::indexToPos(Eigen::Vector3i id, Eigen::Vector3d &pos) 
{
  for (int i = 0; i < 3; ++i)
...
// 检查cur_pos是否安全
bool Dijkstra::check_safety(Eigen::Vector3d &cur_pos, double safe_distance)
{
  bool is_safety;
