...
class WindTurbineRealCoveragePlanner
{
public:
    WindTurbineRealCoveragePlanner(ros::NodeHandle& nh)
    {
        ...
        // 缩小版风机几何默认值（按原大风机约 0.105 倍缩尺）
        nh.param("coverage/tower_radius", tower_radius_, 0.0631);
        nh.param("coverage/tower_height", tower_height_, 1.4306);

        nh.param("coverage/hub_x", hub_x_, 0.0);
        nh.param("coverage/hub_y", hub_y_, -0.1196);
        nh.param("coverage/hub_z", hub_z_, 1.3852);

        // 塔筒螺旋
        nh.param("coverage/tower_offset_distance", tower_offset_distance_, 0.18);
        nh.param("coverage/tower_spiral_pitch", tower_spiral_pitch_, 0.42);
        nh.param("coverage/tower_wp_arc_step", tower_wp_arc_step_, 0.10);
        nh.param("coverage/tower_z_start", tower_z_start_, 0.15);
        nh.param("coverage/tower_z_end", tower_z_end_, 1.20);

        // 叶片螺旋
        nh.param("coverage/blade_root_margin", blade_root_margin_, 0.05);
        nh.param("coverage/blade_tip_margin", blade_tip_margin_, 0.04);
        nh.param("coverage/blade_helix_radius", blade_helix_radius_, 0.18);
        nh.param("coverage/blade_helix_pitch", blade_helix_pitch_, 0.26);
        nh.param("coverage/blade_axis_step", blade_axis_step_, 0.05);

        // 叶片之间的安全转移：保留 pre_entry_safe
        nh.param("coverage/blade_transfer_side_offset", blade_transfer_side_offset_, 0.22);
        nh.param("coverage/blade_transfer_axis_margin", blade_transfer_axis_margin_, 0.05);

        // 叶片 root/tip（按缩尺后的默认值）
        nh.param("coverage/blade1_root_x", blade1_root_x_, 0.0415);
        nh.param("coverage/blade1_root_y", blade1_root_y_, -0.0954);
        nh.param("coverage/blade1_root_z", blade1_root_z_, 1.3875);
        nh.param("coverage/blade1_tip_x",  blade1_tip_x_,  0.6174);
        nh.param("coverage/blade1_tip_y",  blade1_tip_y_, -0.1059);
        nh.param("coverage/blade1_tip_z",  blade1_tip_z_,  1.0345);

        nh.param("coverage/blade2_root_x", blade2_root_x_, -0.0182);
        nh.param("coverage/blade2_root_y", blade2_root_y_, -0.0954);
        nh.param("coverage/blade2_root_z", blade2_root_z_, 1.3470);
        nh.param("coverage/blade2_tip_x",  blade2_tip_x_, -0.6118);
        nh.param("coverage/blade2_tip_y",  blade2_tip_y_, -0.1059);
        nh.param("coverage/blade2_tip_z",  blade2_tip_z_,  1.0248);

        nh.param("coverage/blade3_root_x", blade3_root_x_, -0.0233);
        nh.param("coverage/blade3_root_y", blade3_root_y_, -0.0954);
        nh.param("coverage/blade3_root_z", blade3_root_z_, 1.4185);
        nh.param("coverage/blade3_tip_x",  blade3_tip_x_, -0.0056);
        nh.param("coverage/blade3_tip_y",  blade3_tip_y_, -0.1059);
        nh.param("coverage/blade3_tip_z",  blade3_tip_z_,  2.0942);

        // 起终点
        nh.param("coverage/approach_radius", approach_radius_, 0.80);
        nh.param("coverage/approach_height", approach_height_, 0.30);
        nh.param("coverage/final_hover_radius", final_hover_radius_, 0.80);
        nh.param("coverage/final_hover_height", final_hover_height_, 0.30);

        // 执行
        nh.param("coverage/reach_dist", reach_dist_, 0.08);
        ...
        nh.param("coverage/metrics_min_step", metrics_min_step_, 0.005);
        ...
        drone_sub_ = nh.subscribe(drone_state_topic_, 10, &WindTurbineRealCoveragePlanner::droneStateCb, this);

        timer_ = nh.createTimer(ros::Duration(publish_interval_),
                                &WindTurbineRealCoveragePlanner::timerCb,
                                this,
                                false,
                                false);
    ...
    void start()
    {
        if (wps_.empty())
        {
            ROS_WARN("[real_coverage_planner] waypoint list is empty.");
            return;
        }
        ...
        ROS_INFO("[real_coverage_planner] mission started, total waypoints = %zu", wps_.size());
    }
...
private:
    ...
    void publishGoal(const Eigen::Vector3d& p)
    {
        ...
        ROS_INFO("[real_coverage_planner] send goal [%zu/%zu]: %.2f %.2f %.2f",
    ...
    void printMissionMetrics()
    {
        ...
        ss << "[real_coverage_planner][TOTAL] Arrived. time=" << std::fixed << std::setprecision(2)
           << t
    ...
    void timerCb(const ros::TimerEvent&)
    {
        ...
        if (!drone_ready_)
        {
            ROS_WARN_THROTTLE(2.0, "[real_coverage_planner] waiting for drone state...");
            return;
        }
        ...
        if (current_idx_ >= wps_.size())
        {
            ...
            printMissionMetrics();
            ROS_INFO("[real_coverage_planner] all waypoints finished.");
            return;
        }
                                                   ...
                                                   const Eigen::Vector3d& tip) const
    {
        ...
        info.s0 = std::min(std::max(0.01, blade_root_margin_), info.axis_len * 0.25);
        info.s1 = std::max(info.s0 + 0.06, info.axis_len - blade_tip_margin_);

        info.entry = root + info.dir * info.s0 + blade_helix_radius_ * info.e1;

        info.pre_entry_safe =
            root
          + info.dir * std::max(0.01, info.s0 - blade_transfer_axis_margin_)
          + blade_helix_radius_ * info.e1
          + rotorSafeOffset(blade_transfer_side_offset_);

        const double work_len = std::max(0.0, info.s1 - info.s0);
        const double turns = work_len / std::max(0.05, blade_helix_pitch_);
    ...
    void buildTowerSpiral()
    {
        ...
        const double total_h = z1 - z0;
        const double total_turns = total_h / tower_spiral_pitch_;
        const int total_steps = std::max(n_per_circle, static_cast<int>(std::ceil(total_turns * n_per_circle)));

        ROS_INFO("[real_coverage_planner] tower scan_r = %.3f, total_turns = %.3f", scan_r, total_turns);
    ...
    Eigen::Vector3d appendBladeHelixFromEndpoints(const BladeInfo& blade)
    {
        ...
        if (info.s1 <= info.s0)
        {
            ROS_WARN("[real_coverage_planner] invalid blade endpoints for %s", blade.name.c_str());
            return root;
        }
        ...
        const double turns = work_len / std::max(0.05, blade_helix_pitch_);
        const int steps = std::max(8, static_cast<int>(std::ceil(work_len / std::max(0.01, blade_axis_step_))));

        ROS_INFO("[real_coverage_planner] append %s helix, axis_len=%.3f", blade.name.c_str(), info.axis_len);
    ...
    void buildMission()
    {
        ...
        ROS_INFO("[real_coverage_planner] blade order after tower: %s -> %s -> %s",
        ...
        ROS_INFO("[real_coverage_planner] mission waypoints generated = %zu", wps_.size());
    }
};

int main(int argc, char** argv)
{
    ros::init(argc, argv, "wind_turbine_real_coverage_planner");
    ros::NodeHandle nh("~");

    WindTurbineRealCoveragePlanner node(nh);
