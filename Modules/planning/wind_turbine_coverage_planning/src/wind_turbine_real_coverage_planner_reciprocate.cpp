...
class WindTurbineRealCoveragePlannerReciprocate
{
public:
    explicit WindTurbineRealCoveragePlannerReciprocate(ros::NodeHandle& nh)
    {
        ...
        // 叶片正面 / 反面往复式覆盖
        nh.param("coverage/blade_root_margin", blade_root_margin_, 0.06);
        nh.param("coverage/blade_tip_margin", blade_tip_margin_, 0.05);
        nh.param("coverage/blade_axis_step", blade_axis_step_, 0.08);
        ...
        if (!nh.getParam("coverage/blade_surface_offset", blade_surface_offset_))
        {
            ...
            {
                blade_surface_offset_ = 0.18;
            }
        }

        // 从正/反面扫描线再往外退开的安全余量（同样沿正反面方向）
        nh.param("coverage/blade_transfer_side_offset", blade_transfer_side_offset_, 0.12);
        // 进出叶片时沿叶片轴向预留的缓冲
        nh.param("coverage/blade_transfer_axis_margin", blade_transfer_axis_margin_, 0.05);
        // 叶尖处绕到另一面的外扩量（沿叶片径向外侧）
        nh.param("coverage/blade_tip_turn_offset", blade_tip_turn_offset_, 0.18);
        ...
        drone_sub_ = nh.subscribe(drone_state_topic_, 10,
                                  &WindTurbineRealCoveragePlannerReciprocate::droneStateCb, this);

        timer_ = nh.createTimer(ros::Duration(publish_interval_),
                                &WindTurbineRealCoveragePlannerReciprocate::timerCb,
                                this,
                                false,
                                false);
    ...
    void start()
    {
        if (wps_.empty())
        {
            ROS_WARN("[real_coverage_planner_reciprocate] waypoint list is empty.");
            return;
        }
        ...
        ROS_INFO("[real_coverage_planner_reciprocate] mission started, total waypoints = %zu", wps_.size());
    }
...
private:
    ...
    void publishGoal(const Eigen::Vector3d& p)
    {
        ...
        ROS_INFO("[real_coverage_planner_reciprocate] send goal [%zu/%zu]: %.2f %.2f %.2f",
    ...
    void printMissionMetrics()
    {
        ...
        ss << "[real_coverage_planner_reciprocate][TOTAL] Arrived. time=" << std::fixed << std::setprecision(2)
           << t
    ...
    void timerCb(const ros::TimerEvent&)
    {
        ...
        if (!drone_ready_)
        {
            ROS_WARN_THROTTLE(2.0, "[real_coverage_planner_reciprocate] waiting for drone state...");
            return;
        }
        ...
        if (current_idx_ >= wps_.size())
        {
            ...
            printMissionMetrics();
            ROS_INFO("[real_coverage_planner_reciprocate] all waypoints finished.");
            return;
        }
    ...
    void buildTowerSpiral()
    {
        ...
        ROS_INFO("[real_coverage_planner_reciprocate] tower scan_r = %.2f, total_turns = %.2f",
                 scan_r, total_turns);
    ...
    void buildMission()
    {
        ...
        while (!remaining.empty())
        {
            ...
            ROS_INFO("[real_coverage_planner_reciprocate] choose %s, start_on_%s, transition_dist=%.2f",
        ...
        buildFinalHover();
        ROS_INFO("[real_coverage_planner_reciprocate] mission waypoints generated = %zu", wps_.size());
    }
};

int main(int argc, char** argv)
{
    ros::init(argc, argv, "wind_turbine_real_coverage_planner_reciprocate");
    ros::NodeHandle nh("~");

    WindTurbineRealCoveragePlannerReciprocate node(nh);
