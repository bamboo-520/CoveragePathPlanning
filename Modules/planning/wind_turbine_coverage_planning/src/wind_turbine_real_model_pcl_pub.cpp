...
class WindTurbineRealModelPclPub
{
public:
    WindTurbineRealModelPclPub(ros::NodeHandle& nh)
        : rng_(std::random_device{}())
    {
        ...
        nh.param("support_mesh", support_mesh_,
                 std::string("/home/amov/Prometheus/Simulator/gazebo_simulator/models/wind_turbine/meshes/Support.stl"));
        nh.param("center_mesh", center_mesh_,
                 std::string("/home/amov/Prometheus/Simulator/gazebo_simulator/models/wind_turbine/meshes/Center.stl"));
        nh.param("blade1_mesh", blade1_mesh_,
                 std::string("/home/amov/Prometheus/Simulator/gazebo_simulator/models/wind_turbine/meshes/Blade1.stl"));
        nh.param("blade2_mesh", blade2_mesh_,
                 std::string("/home/amov/Prometheus/Simulator/gazebo_simulator/models/wind_turbine/meshes/Blade2.stl"));
        nh.param("blade3_mesh", blade3_mesh_,
                 std::string("/home/amov/Prometheus/Simulator/gazebo_simulator/models/wind_turbine/meshes/Blade3.stl"));

        // 缩小版模型尺寸更小，所以点云参数也调细一些
        nh.param("sample_density", sample_density_, 12000.0); // 每平方米采样点数
        nh.param("min_points_per_mesh", min_points_per_mesh_, 1200);
        nh.param("leaf_size", leaf_size_, 0.008);             // 体素滤波下采样

        // 如果 world 中整体又平移/旋转了，可通过这些参数做整体位姿补偿
        ...
        timer_ = nh.createTimer(ros::Duration(1.0 / std::max(0.1, publish_rate_)),
                                &WindTurbineRealModelPclPub::timerCb, this);
    }
...
private:
                           ...
                           double& total_area)
    {
        ...
        {
            ROS_ERROR("[wind_turbine_real_model_pcl_pub] failed to load STL: %s", mesh_path.c_str());
            return false;
        }
        ...
        {
            ROS_ERROR("[wind_turbine_real_model_pcl_pub] mesh has no valid triangles: %s", mesh_path.c_str());
            return false;
        }

        ROS_INFO("[wind_turbine_real_model_pcl_pub] loaded %s, triangles=%zu, area=%.6f",
    ...
    void buildCloud()
    {
        ...
        ROS_INFO("[wind_turbine_real_model_pcl_pub] raw points=%zu, filtered points=%zu",
...
{
    ros::init(argc, argv, "wind_turbine_real_model_pcl_pub");
    ros::NodeHandle nh("~");

    WindTurbineRealModelPclPub node(nh);
    ros::spin();
    return 0;
}
